...
class DataHandler:
    ...
    def __get_all_file_paths_in_path__(path: str,
        ...
        exclude = [
            '/home/lschilling/datam2olie/synthetic/native/t3/Synthetic_MR/MR_Model117_Energy100_act_3.nrrd'
        ]
    ...
    def data_gen_mnist(self, data, batch_size=4) -> (np.array, np.array):
    ...
    def data_gen_voxelmorph(
        ...
        while True:
            if idx + batch_size > len(data_x):
                idx = 0
            if shuffle:
                idxs = np.random.randint(0, data_x.shape[0], size=batch_size)
            ...
            if type(data_x[0]) is np.ndarray:
                ...
                fixed_images = data_x[idxs_fixed, ...]
